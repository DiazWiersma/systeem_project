    ...
    try:
        ...
        if chunk_mode:
            ...
            for idx, (word_start, chunk_str) in enumerate(chunks):
                if idx in matched:
                    kw_here = [k for k in keywords if k.lower() in chunk_str.lower()]
                    highlights.append({
                        "chunk_idx": idx,
                        "text": chunk_str,
            ...
            # Heeft het model geen chunks aan dit topic gekoppeld, val dan terug
            # op de zin-gebaseerde markering, zodat de lezer nooit leeg blijft.
            if highlights:
                return {
